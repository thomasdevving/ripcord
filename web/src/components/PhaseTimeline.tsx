/**
 * The phase timeline. Phases are listed in the ENGINE'S REAL ORDER and only
 * those the selected mode actually runs. There is no percentage bar: the engine
 * does not know in advance how many RPC round-trips a role reconstruction will
 * take, and a bar that walks to 100% independently of the work is a progress
 * indicator that lies.
 *
 * FOUR DISTINCT NON-PENDING STATUSES, and the distinctions are load-bearing:
 *   completed    — ran and answered. A zero-result answer counts. This is the
 *                  ONLY green, and it means the step finished, never that the
 *                  contract is safe.
 *   inconclusive — ran and could not answer. First-class here, not a failure.
 *   degraded     — threw, and a fallback value was substituted. Never green.
