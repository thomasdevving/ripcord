 ...
 * only its hash is stored server-side. Anyone handed an analysis link can watch
 * it; only the submitter can stop it.
 *
 * `sessionStorage` rather than `localStorage`, deliberately: the token is useful
 * for exactly as long as the job runs. Every access is wrapped, because storage
 * throws outright in some contexts and a cancel button is not worth taking the
 * page down for.
 */
