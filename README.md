...
The webapp also has a first **protocol timeline** workflow. A user groups up to four related contracts, establishes a block-consistent baseline, and manually rescans them later. Ripcord compares interpreted power, role membership, capability guards, authority routes, exit timing, verdict and analysis coverage against the preceding scan. Evidence ordering and generation timestamps are excluded from the comparison. A withheld, failed or incompatible report is an explicit coverage gap and can never be rendered as “no change.” This is a manual comparison workflow; it does not schedule scans or send alerts.
...
| **Capabilities** | EVMole statically follows calldata and control flow through the runtime bytecode to recover externally dispatched selectors — *not* a linear byte scan, so embedded child bytecode and ordinary in-function `bytes4` comparisons are not misread as entry points. Selectors match a versioned taxonomy into power categories (CODE_CHANGE, FUND_MOVEMENT, SUPPLY, ACCESS_RESTRICTION, ECONOMIC, AUTHORITY_CHANGE). |
...
| **EVM, one chain, one block** | Ethereum Mainnet only. The web resolves `latest` once at submission and pins that number and hash for the whole run; the CLI accepts an explicit historical block. No L2s or non-EVM chains. |
...
| **The fork differential cannot prove safe exit — and never claims to** | `ripcord restrict` runs a real exit-restriction differential on a fork (`src/fork/exitRestriction.ts`): establish a baseline exit, have the current archetype's registered guarding party try to close it, and watch whether the exit then fails. A *found* restrictor is decisive (this is how Comet's guardian becomes a fork-confirmed zero-notice route). A *clean* run is designed to earn only the deliberately weaker `no_direct_restriction_found`, **scoped to the registered candidates evaluated and never a safety guarantee**. It is fail-closed at three independent layers: an `inconclusive`/`not_evaluated` candidate or incomplete aggregate enumeration produces `evaluation_inconclusive` in the engine; the schema rejects a contradictory clean object; and verdict composition repeats the candidate, coverage and enumeration checks. Three limits still apply verbatim: (1) it tests the exit action Ripcord *identified*; (2) it tries one bounded exit-restricting argument per registered candidate and does not yet account for every privileged function; (3) it does **not** model indirect or economic restrictions — oracle manipulation, collateral/liquidity configuration, fee/rate changes, or call sequences. This pass ships **one exit archetype and one registered candidate** (Compound III / Comet base-withdrawal versus `pause(...withdraw=true...)`), validated live. The calibration run finds that restrictor, so the positive tier occurs nowhere in the set and is not presented as empirically validated. |
...
| **It does not monitor continuously** | Protocol workspaces support a manual baseline and manual rescan. There is no scheduler, event watchtower, alert delivery or automated response. Each report remains a photograph of one pinned block. |
...
| **[Better Auth](https://better-auth.com)** | Email/password sessions and organization membership. Customer protocols, scans, jobs and live reports are isolated by the active organization and persisted in SQLite on the mounted volume. |
...
- **Mobula** — the **live exposure** layer, and deliberately the one partner technology that is *not* allowed to touch the verdict. Mobula proposes a fresh, timestamped set of assets that the curated six-token dependency pass could miss. After the deterministic report is stored, an explicit default-off web option selects candidates from a separate unfiltered same-chain response independently of the visible value floor/cap and presentation filters, itemises every selection exclusion, and verifies up to 64 same-chain ERC20 identities with `getCode` and `balanceOf` at the pinned report block. In a selected fork mode, a recognised Compound III target then admits both explicit zero and non-zero balance candidates into a second, sidecar-only differential: seed an isolated fork holder without draining the target, establish a real Compound collateral position, prove the withdrawal works, revert to the same state, invoke the real guardian pause and repeat the identical withdrawal. Every asset receives its own pre-candidate snapshot. A balance alone is never called a fork test, an unrecognised asset is never guessed at, and none of these results can change the core verdict. An optional token-security audit compares four narrow Mobula flags with Ripcord's observed capabilities as regression leads only; it cannot edit a report or verdict. The live panel keeps the Mobula fetch time separate from the pinned block, and `pnpm verify:boundary` walks the import graph to prove no pinned report code can reach the live layer. See **[docs/MOBULA.md](docs/MOBULA.md)**.
...
# 2. Power far larger — and the exit is CLOSABLE. Compound III, proof + fork differential.
...
**The webapp** (`server/` + `web/`) is a private organization workspace: a signed-in member enters an address, watches the analysis happen, follows the fork experiment and gets a report with its evidence attached. Protocols, scans, jobs and live reports carry the active organization ID, and cross-organization lookups return no data. One Railway service, one replica, one public port; `anvil` is spawned transiently by a job worker, binds loopback, and is never exposed. It imports the engine functions directly — it does not shell out to the CLI or parse its output — and the optional progress hooks it uses cannot change a report, which `test/observer.test.ts` asserts by building the same report with and without an observer and comparing the bytes. See **[docs/WEBAPP.md](docs/WEBAPP.md)** and **[docs/RAILWAY.md](docs/RAILWAY.md)**.
...
- **Day 2 — Capabilities.** Dispatcher-based selector extraction (now backed by EVMole's calldata/control-flow analysis); the versioned capability taxonomy; guard attribution **by probing real `eth_call`s** rather than static analysis; weakest-link uncertainty encoded in the schema; a one-level dependency graph over curated major tokens and oracle getters.
...
| `evmole` | Local EVM bytecode analysis for externally dispatched selector extraction. Runtime; its exact version is recorded in every new report. |
...
Everything else is original work written during the event: proxy detection, the selector-analysis adapter and fail-closed result handling, the capability taxonomy, guard probing and the dialect dictionary, recursive authority resolution, the exit-window and time-to-exit models, verdict composition, the enumeration witness, the report schema, the hand-assembled drainer bytecode, the renderer, and the two verifiers.
