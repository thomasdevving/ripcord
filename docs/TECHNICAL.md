...
`pauseGuardian()` setting withdraw-pause to true while preserving the other flags.

The control supplies 50k base tokens, then calls `withdraw(baseToken, uint256.max)`.
It must recover the supplied assets, clear the base principal and supplied balance,
and leave zero debt. Receipt success by itself does not establish an exit. A failed
control returns `baseline_unestablished`, without inventing a closed-exit cause.
After reverting the snapshot, the candidate branch uses matching local block
numbers and timestamps. Confirmation requires the pause flag's false-to-true
transition, a withdrawal revert with `Paused()`, and unchanged holder tokens and
principal. Other failures are inconclusive. A no-effect result must recover the
same assets and clear the position too.

An impersonated controller skips its own authorization. In the historical Comet
report that controller is a 5-of-9 Safe: signatures, transaction guards and modules
were not executed. Any zero-notice conclusion is conditional on it being able to
authorize the call. Unresolved contract-controller notice remains unknown.
See [FORK_VALIDATION.md](FORK_VALIDATION.md) for the new rules' validation status;
the committed calibration predates these changes.
...
- **The DELEGATECALL "unknown" proxy-pattern heuristic is a linear byte scan, not control-flow analysis** (day 1). It correctly skips PUSH-immediate data and Solidity's trailing CBOR metadata blob, but it has no notion of reachability. A factory contract that deploys a proxy via `new SomeProxy(...)` embeds that proxy's full initcode — including its real `DELEGATECALL` — inside its own bytecode, and this scanner cannot tell "code this contract would execute" from "code embedded as another contract's creation bytecode." Demonstrated live: Aave's `PoolAddressesProvider`, not itself upgradeable, comes back `pattern: "unknown"` for exactly this reason. Reported as uncertain, not as a false "not a proxy" — the correct failure mode, but worth knowing about before reading too much into a lone `unknown`. Capability selector extraction is separate code and does not use this heuristic.
- **Capability selector extraction delegates EVM semantics to EVMole, rather than matching local opcode windows** (day 2, replaced after review). EVMole symbolically follows calldata through recovered control flow and distinguishes normal ABI dispatch from selectors routed through fallback logic. Ripcord normalizes both kinds into the externally callable selector surface, records their counts separately, and writes the exact analyser name and version into the report. Regression fixtures cover old and modern Solidity, binary-search dispatch, embedded child initcode, and an ordinary reachable `bytes4` comparison inside a function body. If the analyser recovers no selectors or rejects malformed bytecode, extraction returns `recognized: false` rather than treating an empty set as proof of no callable surface.
- **Ripcord does not report whether a contract has a generic fallback or `receive()` function.** An early heuristic for this was removed rather than shipped: it returned "has a fallback" for every real contract tested, including ones that have none — a flag with no evidence behind it. EVMole can identify specific selectors routed through fallback logic, and those selectors are included in capability enumeration, but Ripcord still makes no separate claim that a generic fallback or `receive()` body exists.
...
- **The proof engine covers exactly one archetype.** `ripcord prove` simulates only `CODE_CHANGE → drain` on an EIP-1967 **transparent** proxy via `ProxyAdmin.upgrade(address,address)` — the path validated live end-to-end. UUPS (`upgradeToAndCall` on the proxy), beacon, and legacy-zos upgrade paths return `produced: false` with a stated reason rather than a guessed simulation. Deliberate depth-over-breadth, not an oversight. Two further caveats on the ones it does run: the drained dollar figure counts only the curated major-token holdings (a **floor**, see above), and because anvil impersonation ignores signatures, a proof whose resolved controller is a **Safe** impersonates the Safe address directly — assuming the Safe can authorize and submit the call; its signatures, transaction guards and modules are not executed. The PAID demo impersonates a plain EOA, so that caveat doesn't apply there.
