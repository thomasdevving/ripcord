...
| Compound Comet cUSDCv3 | `undetermined` | ProxyAdmin → Compound Timelock, `delay() = 172800`, **proven binding** by probe, and withdrawal is synchronous — so the notice side is genuinely 2 days against a 0s exit. It is NOT reassuring anyway: 67 of 67 selectors were never evaluated for privilege while a governor, proxy admin and guardian pointer exist, and one of those selectors is a guarded `pause`. See [CALIBRATION §13](CALIBRATION.md#13-the-flagship-green-result-that-was-wrong). |
...
| `undetermined` | 17 of 26 — the deliberate trade |
...
- **Under-determination is now the dominant error mode.** 17 of 26 calibration protocols return `undetermined`, with named causes: custom authority schemes (5), provider-starved role enumeration (4), Compound's delegator pattern (2 — the marker names the Timelock but the route is not resolved), **an unevaluated privileged surface beside a privileged party (2 — Compound III and Uniswap v3 Factory, see §13)**, an owner contract with no resolvable authority (1), Vyper's unrecognised dispatcher (1), Aave's governance shape (1), a custom access-control error (1). Against **0 false-clean results** this is the right way round, but it should be quoted honestly whenever coverage is described.
